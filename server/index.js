...
async function universalEngine(url, options) {
    const { keyword, contentType, filterType } = options; // filterType: 'sentence' or 'paragraph'
        ...
        const data = await page.evaluate((opts) => {
            ...
            if (opts.contentType === 'text' || opts.contentType === 'both') {
                ...
                if (opts.filterType === 'paragraph') {
                    // PARAGRAPH LOGIC: Find elements containing keyword and get their full text
                    ...
                    // SENTENCE LOGIC: Uses Regex to find keyword and include the trailing punctuation
                    const rawText = container.innerText;
                    // Regex: finds text around keyword ending in . ! or ?
